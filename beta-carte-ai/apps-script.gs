...
//
// - Enregistre chaque inscription (Nom, Contact, Pays) dans la Sheet.
// - Limite a 10 places par pays (MAX_PER_COUNTRY) - au-dela, l'inscription
//   est refusee avec un message "beta complete pour ce pays".
// - Envoie un email de confirmation automatique a l'inscrit (si son contact
//   ressemble a un email), et une notification a l'admin.
//
// Installation :
// 1. Creez une nouvelle Google Sheet, et ajoutez une ligne d'en-tete sur la
//    premiere feuille : Date | Nom | Contact | Pays
...
function doPost(e) {
  try {
    ...
    const countryCount = rows.filter((row) => String(row[3]).trim().toLowerCase() === country.toLowerCase()).length
    ...
    if (contact.indexOf("@") !== -1) {
      sendConfirmationEmail(name, contact, country)
    }
    notifyAdmin(name, contact, country, countryCount + 1)
...
function notifyAdmin(name, contact, country, rankInCountry) {
  try {
